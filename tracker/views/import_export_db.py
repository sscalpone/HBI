...
def move_files(request, pathname):
	...
	if not (basename.endswith('.csv')):
		...
		if (os.path.dirname(pathname).endswith('documents')):
			...
			messages.add_message(request, messages.ERROR, 
				('Unable to read %s subfolder. Please add all csvfiles to '
				 'main folder and re-upload.' % d2))
			shutil.rmtree(pathname)
		else:
			messages.add_message(request, messages.ERROR, 
				('%s is not a csv file. Please only upload csv files.' 
				 % basename))
			os.remove(pathname)
	...
	# conflicting copies message and delete both files
		...
		messages.add_message(request, messages.ERROR, 
			('Conflicting copies of %s uploaded. Please choose one copy and'
			 ' re-upload.' % basename))
		os.remove(pathname)
...
def compare_csv(request, file, model_instance, fix, dependent_csvs_dict):
	...
	for row in reader:
		...
		try:
			...
			if (fix is False):
				if (pytz.utc.localize(datetime.datetime.strptime(
					...
					messages.add_message(request, messages.ERROR, 
						('%s is already up to date in the database, no need '
						 'to update' % model_instance))
			...
			if (uuid_csv != ''):
				if (model_instance == 'user'):
					obj_inst = obj.objects.create_user(row['username'], 
						row['email'], row['password'], uuid=uuid_csv)
				else:
		...
		if dependent_csvs_dict:
			...
			for key, value in dependent_csvs_dict.iteritems():
				...
				if (row[key] != ''):
					...
					if (dependent_uuid == ''):
						messages.add_message(request, messages.ERROR, 
							('There is no %s with the id %s in %s.csv' 
							 % (key, dependent_id, model_instance)))
						return False
		...
		for name in field_names:
			...
			if (row[name]=='' and 
				obj_inst._meta.get_field(name).get_default()):
				...
				if (value != ''):
					if (type(value).__name__ == 'datetime'):
						...
						try: 
							...
							messages.add_message(request, messages.ERROR, 
								('The %s datetime for the %s with the id '
								 '%s cannot be parsed.' % 
								 (name, model_instance, row['id'])))
							break
				...
				if (value != ''):
					if (type(value).__name__ == 'date'):
						...
						try: 
							...
							messages.add_message(request, messages.ERROR, 
								('The %s date for the %s with the id %s cannot '
								 'be parsed.' % 
								 (name, model_instance, row['id'])))
							break
